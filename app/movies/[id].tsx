...
const MovieInfo = ({ label, value }: MovieInfoProps) => (
  <View className="flex-col items-start justify-center mt-5">
    <Text className="text-light-200 font-normal text-sm">{label || "N/A"}</Text>
...
const Id = () => {
  ...
  return (
    <View className="bg-primary flex-1">
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <View className="flex-col items-start justify-center mt-5 px-5">
          ...
          <View className="flex flex-row justify-between w-1/2">
            ...
            <MovieInfo label="Revenue" value={`$${movie?.revenue ? Math.round(movie.revenue / 1_000_000) : 0} million`} />
          </View>
      ...
      <TouchableOpacity
        className="absolute bottom-5 right-0 left-0 mx-5 bg-accent py-3.5 rounded-lg flex flex-row items-center justify-center z-50"
        onPress={router.back}
      >
